...
Note the output format of `get_stored_output_values()`: this will return a list. Depending on whether you specified that you would like full output during sampling or not, this list will have multiple entries or just one.
...
If one defined a model with these two parameters as inputs, a number of samples per parameters of 2, and let the algorithm run for two steps, the following would be the output of `journal.get_stored_output_values()` if one specified full output to be 1:
...
.. code-block:: python
    ...
    journal.get_stored_output_values()[step_number]["name"]
