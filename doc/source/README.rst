...
Here, we explain how to use ABCpy to infere parameters of models, given some observed data sets.
...
.. literalinclude:: ../../examples/backends/dummy/pmcabc_gaussian.py
    ...
    :lines: 5
    :dedent: 4
Now, we want to model the height of humans by a Gaussian or Normal model, which has two parameters: the mean, denoted by :math:`\mu`, and the standard deviation, denoted by :math:`\sigma`. The goal is to use ABC to infere these yet unknown parameters from the information contained in the observed data.
...
Note that this prior, in general, can be a much more complicated structure. Each input parameter for a probabilistic model, defined in **abcpy.continuousmodels** or **abcpy.discretemodels** can be again a probabilistic model. To see how this is implemented, see the 'Advanced' section. In the rest of this section, we will refere to each distribution of the prior as *node*.
...
algorithm of choice. As a pre-requisit, it requires a perturbation kernel. For this, we need to specify all nodes that will be perturbed by a kernel. This is simple in our case, however, for more complicated priors, it will be possible to define different kernels acting on a different set of nodes. Check out the 'Advanced' section to learn more. and a
backend.
...
Please note that it would also be possible to not specify a kernel. In this case, a **DefaultPerturbationKernel** object will be initialized when creating the PMCABC object. However, in general, it is often desirable to define your own kernel.
...
Note that the model is given as a list. This is due to the fact that in ABCpy, it is possible to have multiple models associated with a prior, all giving seperate data sets. To learn more, check the 'Advanced' section.
...
We assume that these grades depend on two priors: the size of the class a student is in, as well as the social background from which the student originates. Both of these quantities can be described using some penalty score. Let's assume that the scores are both normally distributed. However, they also both depend on the location of the school. We, therefore, have an additional prior, saying that the mean of these normal distributions will vary uniformly between two values.

Let us define these quantities and dependencies in ABCpy:

INCLUDE CODE OF PRIOR DEFINITION

Note that the individual parameters given to *Normal* are given as lists. This is to keep a uniform interface for all probabilistic models.

Also, we can imagine that if these two effects were negligible, the grades would be normally distributed around some mean:

INCLUDE CODE FOR MEAN

Therefore, the student will receive some grade which is normally distributed, but this grade is distorted by the other priors defined above. The model which we obtain for the grade of some student hence can be written as:

INCLUDE CODE FOR MODEL

ABCpy not only supports the creating of complicated priors. It is now also possible to have multiple models, each associated with some observed data, that share some parts of their priors.

For example, let us assume that the school gives out scholarships to students. The probabilitiy to receive a scholarship is again following a normal distribution:

INSERT CODE FOR NORMAL DISTRIBUTION OF SCHOLARSHIPS
...
ABCpy currently supports the operations "+", "-", "*", "/" and "**" (the power operator in python).
